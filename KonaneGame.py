...
class KonaneGame:
    ...
    def evaluate(self, board, color, terminal_value=0):
        ...
        valid_moves_opponent = self.generate_all_possible_moves(
            board, self.opponent(color))

        value += (10 * len(valid_moves_color))
        value -= (10 * len(valid_moves_opponent))

        if (color == Tile.P_White):
            if (board.game_board[0][0].piece == Tile.P_White):
                vWhite += 1
            elif (board.game_board[0][0].piece == Tile.P_Black):
                vBlack += 1

            if (board.game_board[0][5].piece == Tile.P_White):
                vWhite += 1
            elif (board.game_board[0][5].piece == Tile.P_Black):
                vBlack += 1

            if (board.game_board[5][0].piece == Tile.P_White):
                vWhite += 1
            elif (board.game_board[5][0].piece == Tile.P_Black):
                vBlack += 1

            if (board.game_board[5][5].piece == Tile.P_White):
                vWhite += 1
            ...
            if (board.game_board[0][0].piece == Tile.P_Black):
                vBlack += 1
            elif (board.game_board[0][0].piece == Tile.P_White):
                vWhite += 1

            if (board.game_board[0][5].piece == Tile.P_Black):
                vBlack += 1
            elif (board.game_board[0][5].piece == Tile.P_White):
                vWhite += 1

            if (board.game_board[5][0].piece == Tile.P_Black):
                vBlack += 1
            elif (board.game_board[5][0].piece == Tile.P_White):
                vWhite += 1

            if (board.game_board[5][5].piece == Tile.P_Black):
                vBlack += 1
