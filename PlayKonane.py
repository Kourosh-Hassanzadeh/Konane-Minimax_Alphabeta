...
class PlayKonane:
    ...
    def tile_clicked(self, row, col):
        ...
        if self.board.is_opening_move():
            ...
            self.valid_moves = self.game_manager.get_moves_at_tile(
                self.board, new_tile, self.current_player)
            ...
            self.board_view.set_status(
                "Tile `" + str(new_tile.row) + "," + str(new_tile.col) + "` selected")
            self.selected_tile = new_tile

            self.board_view.draw_tiles(
                board=self.board.game_board)  # Refresh the board
        ...
        # If we already had a piece selected and we are moving a piece
            ...
            winner = self.game_manager.find_winner(
    ...
    def execute_computer_move(self, agent):
        ...
        move = agent.do_min_max(self.board, 0, 0)
